...
$file = 'final-customer-price-edited.csv';
...
if (file_exists($file)) {
    ...
    foreach ($csvData as $row => $data) {
        if($counter!=1)
        {
            $accounts_no_array[] = $data[0];
            $product_skus[] = $data[1];
            $account_sku_price_array[$data[0].'-'.$data[1]] = $data[3];
    ...
    $customer_collection = $customerObj->addAttributeToSelect('*')
        ->addAttributeToFilter('cus_account_no', array("in" => $accounts_no_array));
    ...
    $outputFile = "final-customer-price-edited-". date('Ymd_His').".csv";
